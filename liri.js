...
if(functionToRun=='concert-this'){
    ...
    function concertThis(command){
        ...
        .then(function(response){
            console.log("Concert This: ", response.data)
        })
        .catch(function(err){
            console.log(err)
        })
...
if(functionToRun=='do-what-it-says'){
    ...
    fs.readFile('random.txt', function(err,data){
        if(err) throw err
        arrayOfLines = data.toString().split("\n")
        var numberOfLines = arrayOfLines.length
        console.log("Number of Lines: ",numberOfLines)
        var randomLine = Math.floor(Math.random()*numberOfLines)
        console.log("Array of Lines: ",arrayOfLines)
        console.log("Random Line: ",randomLine)
        var randomCommand = arrayOfLines[randomLine]
        console.log('Random Command Issued: ',randomCommand)
        var splitCommand = randomCommand.split(' ')

        if(splitCommand[0]=='movie-this'){
    ...
    movieThis(command)
        ...
        if(splitCommand[0]=='spotify-this'){
            var songTitleArray = splitCommand.splice(1,5).join(" ")
            console.log("Song Title Array: ",songTitleArray)
            var command = songTitleArray
